...
@section('title')Bantuan Pengembangan dan Modal Usaha @endsection
        ...
        <div class="page-title-box">
            <div class="btn-group pull-right">
                <ol class="breadcrumb hide-phone p-0 m-0">
                    ...
                    <li class="active">
                        Bantuan Pengembangan dan Modal Usaha
                    </li>
                </ol>
            </div>
            <h4 class="page-title">Bantuan Pengembangan dan Modal Usaha</h4>
        </div>
    </div>
</div>
<div class="row">
    <div class="col-sm-12">
        <div class="panel">
            <div class="panel-heading">
                <h3 class="panel-title">Tabel Bantuan Pengembangan dan Modal Usaha</h3>
            </div>
            <div class="panel-body">
                <a href="{{ url('modal/create') }}" class="btn btn-primary"><span class="fa fa-plus"></span> Tambah Data</a>
                <a href="{{ url('modal/print') }}" class="btn btn-inverse"><span class="fa fa-print"></span> Print</a>
                <a href="{{ url('modal/export') }}" class="btn btn-inverse"><span class="fa fa-download"></span> Export</a>
            </div>
            <div class="panel-body">
                    ...
                    <tbody>
                        ...
                        @php($count_roadmap = $roadmap->where('modal_id','=',$model->id)->count())
                            <tr class="{{ $model->kode == '' ? 'danger':null }}">
                                ...
                                <td>
                                    <a href="{{ url('modal/roadmap/'.$model->id) }}" class="btn {{ $count_roadmap > 0 ? 'btn-success':'btn-primary'}} btn-primary">{{ $count_roadmap }} Data Roadmap</a>
                                </td>
                                <td>
                                    <a href="{{ url('modal/'.$model->id) }}" class="btn btn-info btn-sm btn-block"><span class="fa fa-eye"></span> Lihat</a>
                                    <a href="{{ url('modal/'.$model->id.'/edit') }}" class="btn btn-sm btn-warning btn-block"><span class="fa fa-pencil-square"></span> Edit</a>
...
$('.btn.btn-sm.btn-primary.btn-block').on('click', function(){
    var id = $(this).attr('id');
    $('#popupLaporan').html('<form action=modal/create/laporan/'+ id +' method=post>{{ csrf_field() }}<div class=form-group><input class=form-control name=judul placeholder="Judul Laporan"required></div><button class="btn btn-primary" type=submit>Submit</button></form>');
});
function cekKode(){
