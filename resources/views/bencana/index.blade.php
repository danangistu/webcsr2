...
@section('title')Bantuan Bencana Alam @endsection
    ...
    <div class="col-sm-12">
        <div class="panel">
            <div class="panel-heading">
                <h3 class="panel-title">Tabel Bantuan Bencana Alam</h3>
            </div>
            <div class="panel-body">
                <a href="{{ url('bencana/create') }}" class="btn btn-primary"><span class="fa fa-plus"></span> Tambah Data</a>
                <a href="{{ url('bencana/print') }}" class="btn btn-inverse"><span class="fa fa-print"></span> Print</a>
                <a href="{{ url('bencana/export') }}" class="btn btn-inverse"><span class="fa fa-download"></span> Export</a>
            </div>
            <div class="panel-body">
                    ...
                    <tbody>
                        ...
                        @php($count_pemberian = $pemberian->where('bencana_id','=',$model->id)->count())
                            <tr class="{{ $model->kode == '' ? 'danger':null }}">
                                ...
                                <td>
                                    <a href="{{ url('bencana/pemberian/'.$model->id) }}" class="btn {{ $count_pemberian > 0 ? 'btn-success':'btn-primary'}} btn-primary">{{ $count_pemberian }} Data pemberian</a>
                                </td>
                                <td>
                                    <a href="{{ url('bencana/'.$model->id) }}" class="btn btn-info btn-sm btn-block"><span class="fa fa-eye"></span> Lihat</a>
                                    <a href="{{ url('bencana/'.$model->id.'/edit') }}" class="btn btn-sm btn-warning btn-block"><span class="fa fa-pencil-square"></span> Edit</a>
...
$('.btn.btn-sm.btn-primary.btn-block').on('click', function(){
    var id = $(this).attr('id');
    $('#popupLaporan').html('<form action=bencana/create/laporan/'+ id +' method=post>{{ csrf_field() }}<div class=form-group><input class=form-control name=judul placeholder="Judul Laporan"required></div><button class="btn btn-primary" type=submit>Submit</button></form>');
});
function cekKode(){
