...
<div class="row">
    <div class="col-sm-12">
        <div class="page-title-box">
            ...
            <h4 class="page-title">Partisipasi Hari Besar</h4>
        </div>
    </div>
</div>
<div class="row">
    <div class="col-sm-12">
        <div class="panel">
            <div class="panel-heading">
                <h3 class="panel-title">Tabel Partisipasi Hari Besar</h3>
            </div>
            <div class="panel-body">
                <a href="{{ url('hari-besar/create') }}" class="btn btn-primary"><span class="fa fa-plus"></span> Tambah Data</a>
                <a href="{{ url('hari-besar/print') }}" class="btn btn-inverse"><span class="fa fa-print"></span> Print</a>
                <a href="{{ url('hari-besar/export') }}" class="btn btn-inverse"><span class="fa fa-download"></span> Export</a>
            </div>
            <div class="panel-body">
                    ...
                    <tbody>
                        @foreach($models as $model)
                            <tr class="{{ $model->kode == '' ? 'danger':null }}">
                                ...
                                <td>
                                    <a href="{{ url('hari-besar/'.$model->id) }}" class="btn btn-info btn-sm btn-block"><span class="fa fa-eye"></span> Lihat</a>
                                    <a href="{{ url('hari-besar/'.$model->id.'/edit') }}" class="btn btn-sm btn-warning btn-block"><span class="fa fa-pencil-square"></span> Edit</a>
...
$('.btn.btn-sm.btn-primary.btn-block').on('click', function(){
    var id = $(this).attr('id');
    $('#popupLaporan').html('<form action=hari-besar/create/laporan/'+ id +' method=post>{{ csrf_field() }}<div class=form-group><input class=form-control name=judul placeholder="Judul Laporan"required></div><button class="btn btn-primary" type=submit>Submit</button></form>');
});
function cekKode(){
